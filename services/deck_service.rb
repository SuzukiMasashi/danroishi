class DeckService
  ...
  def initialize(
    ...
    @deck_rows     = CSV.read(@deck_file_path, headers: :first_row)
  end
  ...
  def export_deck
    CSV.open(export_deck_file_path, "wb", headers: export_headers, write_headers: true) do |csv|
  ...
  def export_deck_file_path
    "valid_#{File.basename(@deck_file_path, '.*')}.csv"
  end
